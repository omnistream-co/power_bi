...
planogram_data AS (
  SELECT
    ...
    COALESCE(
        (SELECT items.value ->> 'shelf'
         FROM jsonb_array_elements(shelf_element -> 'items') WITH ORDINALITY AS items(value, idx)
         ORDER BY items.idx
         LIMIT 1),
        CONCAT(osp.id, '_', bay ->> 'bayNo', '_', shelf_index)
    ) AS shelf_id
  FROM
    output_store_pog osp
  CROSS JOIN LATERAL jsonb_array_elements(CAST(osp.pog AS jsonb) -> 'planogram' -> 'bays') AS bay
  CROSS JOIN LATERAL jsonb_array_elements(bay -> 'shelves') WITH ORDINALITY AS shelf(shelf_element, shelf_index)
  WHERE osp.base_pog_id = '{{bpid}}'  
...
DistinctItems AS (
    ...
    LEFT JOIN planogram_data pd ON 
        di.store_code = pd.store_code AND
        di.shelf_id_orig = pd.shelf_id
),
ProductsPerShelf AS (
    SELECT DISTINCT
        base_pog_id,
...
final_data_with_min_width AS (
    ...
    LEFT JOIN 
        MinWidthCTE mw ON cd.variant = mw.variant AND cd.cdt1 = mw.cdt1
),
...
shelf_classification AS (
    ...
    FROM planogram_data pd
    LEFT JOIN shelf_agg sa
        ON sa.store_code = pd.store_code
...
shelf_data AS (
    SELECT 
        ...
        LEAST(  -- Add LEAST to cap at shelf width
            ...
            COALESCE(
                SUM(
                    CASE 
                        WHEN fdwm.facings_rows = 0 THEN 0
                        ELSE COALESCE(
                            fdwm.facings * fdwm.merch_width,  -- Changed calculation
                            0
                        )
                    END
                ),
    ...
    FROM 
        planogram_data pd
    LEFT JOIN final_data_with_min_width fdwm ON 
        pd.store_code = fdwm.store_code AND 
        pd.shelf_id = fdwm.shelf_ids
    GROUP BY 
        pd.store_code,
        pd.shelf_id,
        pd.planogram_bays_bayNo,
...
final_shelf_data AS (
   SELECT 
       ...
       CASE 
           WHEN sd.planogram_bays_shelves_width - sd.width_occupied > mwf.min_width_on_cdt1_floating_shelf 
           AND mwf.min_width_on_cdt1_floating_shelf > 0 THEN 1
           ELSE 0
       END AS flag_floating_shelf,
       CASE 
           WHEN sd.planogram_bays_shelves_width - sd.width_occupied > mwa.min_width_on_cdt1
           AND mwa.min_width_on_cdt1 = 0 THEN 1
           ELSE 0
       END AS flag,
       CASE 
           WHEN sd.planogram_bays_shelves_width - sd.width_occupied > mwf.min_width_on_cdt1_floating_shelf THEN 1
           ELSE 0
       END AS potential_floating_shelf,
       1.0 / COUNT(*) OVER (
           PARTITION BY sd.store_code,
           CASE 
               WHEN sd.planogram_bays_shelves_width - sd.width_occupied > mwf.min_width_on_cdt1_floating_shelf 
               AND mwf.min_width_on_cdt1_floating_shelf > 0 THEN 1
               ELSE 0
           END
       ) AS count_stores_floating_shelf,
       COUNT(*) OVER (
           PARTITION BY sd.store_code,
           CASE 
               WHEN sd.planogram_bays_shelves_width - sd.width_occupied > mwf.min_width_on_cdt1_floating_shelf 
               ...
               ELSE 0
           END
       ) AS count_shelves_floating_shelf,
       1.0 / COUNT(*) OVER (
           PARTITION BY sd.store_code,
           CASE 
               WHEN sd.planogram_bays_shelves_width - sd.width_occupied > mwa.min_width_on_cdt1
   ...
   FROM 
       shelf_data sd
   LEFT JOIN
       min_width_cdt1_floating mwf ON sd.shelf_ids = mwf.shelf_ids
   LEFT JOIN
       min_width_cdt1_all mwa ON sd.shelf_ids = mwa.shelf_ids
   LEFT JOIN
       shelf_classification sc ON sd.shelf_unique_id = sc.shelf_unique_id
)

SELECT * 
FROM final_shelf_data
ORDER BY 
    store_code, 
