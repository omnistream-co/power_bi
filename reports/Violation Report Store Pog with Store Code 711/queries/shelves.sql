WITH params AS (
				    ...
				    SELECT filter_config_id, base_pog_id
				    FROM reporting.v_merchflow_projects_temp
				)
				...
				FROM reporting.mv_planogram_data pd
				    -- mv_osp_latest already filters is_latest_version = TRUE
				    -- Join to get filter_config_id and base_pog_id for scoping
				    INNER JOIN reporting.mv_osp_latest osp
				        ON osp.id = pd.id
				    INNER JOIN params p
				        ON osp.filter_config_id = p.filter_config_id
